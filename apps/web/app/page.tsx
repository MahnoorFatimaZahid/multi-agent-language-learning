...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.beginPath();
      const baseRadius  = isActive ? 110 : 125;
  ...
  return (
    <div className="min-h-screen w-screen bg-[#020205] text-white font-sans antialiased flex flex-col justify-between relative overflow-x-hidden"
      style={{ padding: "clamp(16px, 4vw, 48px)" }}
    >
      {/* Noise overlay */}
      <div
        className="absolute inset-0 pointer-events-none z-40 opacity-60"
        style={{
      ...
      {/* Header */}
      <header className="relative z-50 flex items-center justify-between w-full max-w-7xl mx-auto gap-3">
        {/* Logo */}
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${isActive ? "bg-indigo-400 animate-ping" : "bg-indigo-500 animate-pulse"}`} />
        ...
        <div className="flex items-center gap-2 sm:gap-6 flex-wrap justify-end">
          {/* Language switcher — hide labels on very small screens */}
          <div className={`bg-neutral-900/60 border ${borderGlow} p-1 rounded-full flex gap-0.5 backdrop-blur-md transition-all`}>
            {["Spanish", "French", "German", "Japanese"].map((lang) => (
              <button
                key={lang}
                onClick={() => setCurrentLang(lang)}
                className={`text-[10px] uppercase tracking-wider px-2.5 py-1.5 rounded-full transition-all ${
                  currentLang === lang
          ...
          <Link
            ...
            className="text-[10px] sm:text-xs uppercase tracking-widest text-neutral-400 hover:text-white transition-colors whitespace-nowrap"
          >
      ...
      {/* Hero */}
      <div className="relative w-full flex-grow flex flex-col items-center justify-center max-w-4xl mx-auto py-8">

        {/* Text — stacks above canvas on mobile */}
        <div className="w-full flex flex-col items-center text-center gap-6 mb-6 md:mb-0 md:absolute md:inset-0 md:flex-col md:justify-between md:py-12 md:pointer-events-none md:z-10">
          <h1
            className="font-light tracking-tighter leading-none text-white max-w-xl"
          ...
          <p className="text-xs sm:text-sm text-neutral-500 font-light max-w-xs sm:max-w-sm tracking-wide leading-relaxed">
            {isActive
        ...
        >
          ...
          <div className="absolute pointer-events-none text-center flex flex-col items-center justify-center">
            <p className={`text-[11px] font-bold tracking-[0.25em] uppercase drop-shadow-md ${isActive ? "text-indigo-500" : "text-white"}`}>
              {isActive ? "LISTENING" : "TAP TO START"}
            </p>
            <p className="text-[9px] font-mono tracking-widest text-neutral-400/80 mt-1 uppercase">
              {/* {isActive ? `Speaking ${currentLang}` : "Tap to speak"} */}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Footer */}
      <footer className="relative z-50 w-full max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-white/[0.03]">
        ...
        <Link
          ...
          className="px-6 py-3 text-xs uppercase tracking-widest font-bold bg-white text-black rounded-full hover:bg-neutral-100 transition-all whitespace-nowrap"
